...
export const SupabaseService = {
  ...
  restaurants: {
    ...
    // GEOLOCATION: Finding restaurants within 1km walking distance
    async getNearbyDeals(userLat: number, userLng: number, radiusKm: number = 1.0) {
      const { data: nearby, error } = await supabase.rpc('get_nearby_restaurants', {
        user_lat: userLat, user_lng: userLng, radius_km: radiusKm
      });
  ...
  couriers: {
    ...
    async setAvailability(courierId: string, isActive: boolean) {
      return await supabase.from('couriers').update({ is_active: isActive }).eq('id', courierId);
    }
  },
  ...
  orders: {
    async placeOrder(orderData: { 
      user_id: string; restaurant_id: string; service_type: ServiceType; 
      total: number; delivery_fee?: number; promoter_id?: string; notes?: string;
    }, items: any[]) {
      // Step 1: Insert Master Order
      const { data: order, error: orderErr } = await supabase.from('orders').insert([orderData]).select().single();
      if (orderErr) throw orderErr;

      // Step 2: Batch Insert Items with JSONB Options
      ...
      // Step 3: Automated Loyalty & Commission Triggers
  ...
  loyalty: {
    async awardPoints(userId: string, amount: number, reason: string) {
      const { data, error } = await supabase.rpc('increment_loyalty', { user_id: userId, amount: amount });
      // SECURITY: Audit trail for points
      await supabase.from('audit_logs').insert([{
        actor_id: userId, action: `POINTS_EARNED: ${reason}`,
        entity_type: 'loyalty_points', entity_id: userId
      }]);
      return { data, error };
    },
    ...
    async calculateCommission(orderId: string, promoterId: string, orderTotal: number) {
      ...
      if (commission > 0) {
        ...
        return await supabase.from('transactions').insert([{
          order_id: orderId, amount: commission, status: 'success', currency: 'AED'
        }]);
      }
    },
  ...
  // --- 7. MARKETING, ADS & DISCOVERY ---
  marketing: {
    async getActiveAds(placement: AdPlacement = 'home_screen') {
      return await supabase.from('ads').select('*').eq('is_active', true)
        .eq('placement', placement).gt('ends_at', new Date().toISOString());
    },
  ...
  discovery: {
    async logSearch(userId: string, query: string, resultCount: number) {
      return await supabase.from('search_analytics').insert([{ user_id: userId, query, results_count: resultCount }]);
    },
  ...
  support: {
    ...
    async openSupportTicket(userId: string, subject: string, orderId?: string) {
      return await supabase.from('support_tickets').insert([{ user_id: userId, subject, order_id: orderId, status: 'open' }]);
    },
